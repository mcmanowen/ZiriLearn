...
public class AnimalGameScene extends BaseScene  implements IOnSceneTouchListener, IOnAreaTouchListener  {
    ...
    static final String[] textImgsFr = {"ours","chat","coq", "chien","dauphin","canard","aigle","éléphant","gazelle","oie","poule","cheval","lion", "singe","panthère","perroquet","rhinocéros","serpent","écureuil","cygne","tigre","paon","giraffe","lionne","loup","koala","léopard","sanglier","renard","mouton","merle","lapin","faucon","cerf","ane","flamant","lezard","mouette","mante-religieuse","zebre","cigogne","mésange bleue","biche","chardonneret","crabe","taureau","ours polaire","gorille","chamois"};
    static final String[] textImgsFrRead = {"l'oursse","le cha","le cock", "le chien","le dauphin","le canard","l'aigle","l'éléphant","la gazelle","l'oie","la poule","le cheval","le lion", "le singe","la panthère","le perroquet","le rhinocérosse","le serpent","l'écureuil","le cygne","le tigre","le pan","la giraffe","la lionne","le lou","le koala","le léopard","le sanglier","le renard","le mouton","la merle","le lapin","le faucon","le cerre","l'ane","le flamant","le lézard","la mouètte","la mante religieuse","le zébre","la cigogne","la mésange bleue","la biche","le chardonneret","le crabe","le taureau","l'ours polaire","le gorille","le chamois"};
    static final String[] textImgsEn = {"bear","cat","rooster", "dog","dolphin","duck","eagle","elephant","gazelle","goose","hen","horse","lion", "monkey","panther","parrot","rhino","snake","squirrel","swan","tiger","peacock","giraffe","lioness","wolf","koala","leopard","boar","fox","sheep","blackbird","rabbit","falcon","deer","donkey","flamingos","lizard","seagull","european mantis","zebra","stork","bluetit","doe","goldfinch","crab","bull","polar bear","gorilla","chamois"};
    static final String[] textImgsEnRead = {"bear","cat","rooster", "dog","dolphin","duck","eagle","elephant","gazelle","goose","hen","horse","lion", "monkey","panther","parrot","rhino","snake","squirrel","swan","tiger","peacock","giraffe","lioness","wolf","koala","leopard","boar","fox","sheep","blackbird","rabbit","falcon","deer","donkey","flamingos","lizard","seagull","european mantis","zebra","stork","bluetit","doe","goldfinch","crab","bull","polar bear","gorilla","chamois"};
...
*/
	...
	private void goMenu() {
		...
		detachChild(gamecompleteHUD);
		pageCounter = 100;
		resourcesManager.unloadAnimalGameResources();
		//disposeScene();
		}catch(Exception ex){
			ex.printStackTrace();
		}	
	...
	private void readQuestion(int simg){
		...
		if(muteOrNot==false){
			if (localLanguage==0){
			resourcesManager.tts.speak("trouvé, "+textImgsFrRead[simg],TextToSpeech.QUEUE_FLUSH,null);
			}else if(localLanguage==1){
			resourcesManager.tts.speak("where is the, "+textImgsEnRead[simg],TextToSpeech.QUEUE_FLUSH,null);
			}
		}else{}
	}
