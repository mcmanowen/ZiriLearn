...
public class ShapeGameScene extends BaseScene  implements IOnSceneTouchListener, IOnAreaTouchListener  {
    ...
    static final String[] textImgsFr = {"cercle","carré","rectangle","triangle","ovale","parallélogramme","coeur","pentagone","octagone","étoile", "sphère","cone","cube", "cylindre"};
    static final String[] textImgsFrRead = {"cercle","carré","rectangle","triangle","ovale","parallélogramme","coeur","pentagone","octagone","étoile", "sphère","cone","cube", "cylindre"};
    static final String[] textImgsEn = {"circle","square","rectangle","triangle","oval","parallelogram","heart ","pentagon","octagon","star","sphere","cone","cube", "cylinder"};
...
*/
	...
	private void goMenu() {
		...
		detachChild(gamecompleteHUD);
		pageCounter = 100;
		resourcesManager.unloadShapeGameResources();
		//disposeScene();
		}catch(Exception ex){
			ex.printStackTrace();
		}	
	...
	private void readQuestion(int simg){
		...
		if(muteOrNot==false){
			if (localLanguage==0){
			resourcesManager.tts.speak("trouvé la forme, "+textImgsFrRead[simg],TextToSpeech.QUEUE_FLUSH,null);
			}else if(localLanguage==1){
			resourcesManager.tts.speak("where is the, "+textImgsEn[simg],TextToSpeech.QUEUE_FLUSH,null);
			}
		}else{}
	}
