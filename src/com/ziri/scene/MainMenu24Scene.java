...
public class MainMenu24Scene extends BaseScene implements IOnMenuItemClickListener {
    ...
    //--texts
    static final String[] textsFr = {"2-4 ans : leçons","2-4 ans : jeux","Options","Profil","Jeux","Leçons","Alphabets", "Chiffres","Couleurs","Animaux","Musique","Aliments","ziri ballons"};
    //static final String[] textsFrRead = {"Choisissez Un Profile SVP !","0-2 ans","2-4 ans", "+4 ans"};
    static final String[] textsEn = {"2-4 years : learn","2-4 years : games","Options","Profile","Games","Learn","Alphabets", "Numbers","Colors","Animals","Music","Food","ziri balls"};
	...
	private final int TXT_ANIMAL = 9;
	private final int TXT_MUSIC = 10;
	private final int TXT_FOOD = 11;
...
private void createMenuChildScene() {
	...
	*/
	final Text bitmapTextAnimalLearn = new Text(100, 64, resourcesManager.mBitmapFont, getText(TXT_ANIMAL), vbom);		
	menuChildScene.attachChild(bitmapTextAnimalLearn);
	
	final Text bitmapTextMusicalinstLearn = new Text(290, 64, resourcesManager.mBitmapFont, getText(TXT_MUSIC), vbom);		
	menuChildScene.attachChild(bitmapTextMusicalinstLearn);
	
	final Text bitmapTextFoodLearn = new Text(480, 64, resourcesManager.mBitmapFont, getText(TXT_FOOD), vbom);		
	...
	*/
	...
	final Text bitmapTextOptions = new Text(700, 350, resourcesManager.mBitmapFont, "Options", vbom);		
	menuChildScene.attachChild(bitmapTextOptions);
	
	final Text bitmapTextProfile = new Text(700, 192, resourcesManager.mBitmapFont, "Profile", vbom);		
	menuChildScene.attachChild(bitmapTextProfile);
	
	final Text bitmapTextPlay = new Text(700, 36, resourcesManager.mBitmapFont, "Play", vbom);		
	...
	animallearnMenuItem.setPosition(100, 140);
	musicalinstlearnMenuItem.setPosition(290, 140);
	foodlearnMenuItem.setPosition(480, 140);	
...
private void createMenuPlayChildScene() {
	...
	*/
	final IMenuItem animallearnMenuItem = new ScaleMenuItemDecorator(new SpriteMenuItem(MENU_ANIMALPLAY, resourcesManager.btAnimalplayIconTR, vbom), 1f, .8f);
	final IMenuItem musicalinstlearnMenuItem = new ScaleMenuItemDecorator(new SpriteMenuItem(MENU_MUSICALINSTPLAY, resourcesManager.btMusicalinstplayIconTR, vbom), 1f, .8f);
	final IMenuItem foodlearnMenuItem = new ScaleMenuItemDecorator(new SpriteMenuItem(MENU_FOODPLAY, resourcesManager.btFoodplayIconTR, vbom), 1f, .8f);
	final IMenuItem ballgameMenuItem = new ScaleMenuItemDecorator(new SpriteMenuItem(MENU_BALLGAME, resourcesManager.btGame24IconTR, vbom), 1f, .8f);
...
public SceneType getSceneType() {
	return SceneType.SCENE_MAINMENU24;
}
...
public boolean onMenuItemClicked(MenuScene pMenuScene, IMenuItem pMenuItem,
	...
	switch(pMenuItem.getID())
	{
		...
		case MENU_BALLGAME:
			...
			SceneManager.getInstance().loadBallGame24Scene(engine);				
			return true;
		...
		case MENU_ANIMALPLAY:
			...
			pauseMusic();
			resourcesManager.setIdMenu(1);
			SceneManager.getInstance().loadAnimalGameScene(engine);					
			return true;		
		case MENU_FOODPLAY:				
			playSelectSound();
			pauseMusic();
			resourcesManager.setIdMenu(1);
			SceneManager.getInstance().loadFoodGameScene(engine);
			return true;
		case MENU_MUSICALINSTPLAY:	
			playSelectSound();
			pauseMusic();
			resourcesManager.setIdMenu(1);
			SceneManager.getInstance().loadMusicinstrGameScene(engine);
			return true;
